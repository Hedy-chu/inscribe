...
contract InscribeFactory {
    ...
    modifier checkMaxInscribe(address insAddr){
        require(IInscribeERC20(insAddr).totalSupply() + _perMint[insAddr] <= _maxInscribe[insAddr],"maxInscribe");
        _;
    }


    function depolyInscription(string memory name, string memory symbol, uint256 totalSupply, uint256 perMint) public returns (address) {
    ...
    function getBalance(address insAddr,address user) public  returns (uint256) {
